...
export const ApirisTelemetryHeader: React.FC<ApirisTelemetryHeaderProps> = ({
  ...
  const riskWeight = apiris?.risk_weight ?? (latestDecision?.verdict === 'BLOCK' ? 1.0 : latestDecision?.verdict === 'FLAG' ? 0.35 : 0.02);
  const riskTier = apiris?.risk_classification ?? (latestDecision?.verdict === 'BLOCK' ? 'CRITICAL' : latestDecision?.verdict === 'FLAG' ? 'HIGH' : 'LOW');
  ...
  return (
    <div className="w-full bg-[#0A0A0C] border-b border-white/10 px-4 py-2 flex flex-wrap items-center justify-between gap-3 text-xs font-mono text-[#C5C0B7] z-40 sticky top-0 shadow-lg backdrop-blur-md bg-opacity-95">
      {/* Left: Engine & Telemetry Title */}
      <div className="flex items-center gap-2.5">
        <div className="flex items-center gap-1.5 bg-[#161619] px-2.5 py-1 rounded border border-white/10">
          <Activity size={13} className="text-[#D4A15C] animate-pulse" />
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {isAllow ? <ShieldCheck size={13} /> : isFlag ? <AlertTriangle size={13} /> : <ShieldAlert size={13} />}
          <span>{verdict}</span>
        </div>
      </div>
    </div>
